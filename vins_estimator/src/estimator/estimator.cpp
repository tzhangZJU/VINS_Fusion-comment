...
void Estimator::inputImage(double t, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    if(MULTIPLE_THREAD)  
    {     
        if(inputImageCnt % 2 == 0)  //多线程的时候，仅使用偶数计数的图像特征？ tzhang
        {
            mBuf.lock();
        ...
        if(!featureBuf.empty())
        {
            ...
            if(USE_IMU)
            {
                if(!initFirstPoseFlag)
                    initFirstIMUPose(accVector);  //基于重力，对准第一帧
...
void Estimator::initFirstIMUPose(vector<pair<double, Eigen::Vector3d>> &accVector)  //利用重力信息，初始化初始的旋转矩阵
{
...
void Estimator::processIMU(double t, double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (!pre_integrations[frame_count])  // 滑窗中之前不存在该预积分，然后新建； pre_integrations为存储指针的数组
    {
    ...
    if (frame_count != 0)
    {
        ...
        Vs[j] += dt * un_acc;
    }  //此处的acc_0和gyr_0定义在estimator.h中,注意与integration_base.h中的区别  tzhang
    acc_0 = linear_acceleration;
    gyr_0 = angular_velocity; 
}
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header)
{
    ...
    {
        marginalization_flag = MARGIN_SECOND_NEW;  //当前帧为非关键帧，则边缘化当前帧 （PS：称frame_count为当前帧，可能不是很妥当） tzhang
        //printf("non-keyframe\n");
    }
        ...
        if (!STEREO && USE_IMU)
        {
            if (frame_count == WINDOW_SIZE)  //滑窗满后才开始初始化 tzhang
            {
                ...
                if(ESTIMATE_EXTRINSIC != 2 && (header - initial_timestamp) > 0.1)
                {
                    ...
                    initial_timestamp = header;  //更新计算过初始化的时间戳
                }
                if(result)  //初始化成功
                {
                    optimization();
    ...
    else  //初始化成功，优化环节 tzhang
    {
        ...
        last_R0 = Rs[0];
        last_P0 = Ps[0];
        updateLatestStates();  //基于中值积分，计算位姿
    }  
}
...
void Estimator::optimization()
{
    ...
    for (int i = 0; i < frame_count + 1; i++)  //IMU场景，frame_count等于WINDOW_SIZE才会调用optimization() tzhang
    {
    ...
    if (last_marginalization_info && last_marginalization_info->valid)  //边缘化残差，last_marginalization_parameter_blocks怎么得到的？ why tzhang
    {
    ...
    int f_m_cnt = 0;
    int feature_index = -1;
    for (auto &it_per_id : f_manager.feature)  //对每个路标点进行轮询
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)  //对每个路标点在各个图像上的特征点进行轮询
        {
            imu_j++;
            if (imu_i != imu_j)
            {
    ...
    TicToc t_whole_marginalization;   //滑窗满了，进行边缘化处理
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        if (last_marginalization_info && last_marginalization_info->valid)
        {
            ...
            {
                if (last_marginalization_parameter_blocks[i] == para_Pose[0] ||  //将最老的图像帧数据边缘化； tzhang
...
{
    TicToc t_margin;
    if (marginalization_flag == MARGIN_OLD)  // 边缘化最好的图像帧，即次新的图像帧为关键帧
    {
...
void Estimator::slideWindowOld()
{
    ...
    if (shift_depth)
    {
        Matrix3d R0, R1;  //R0、P0表示被边缘化的图像帧，即老的第0帧的位姿； R1、P1表示边缘化后的图像帧中，即新的第0帧的位姿 tzhang
        Vector3d P0, P1;
        R0 = back_R0 * ric[0];
        R1 = Rs[0] * ric[0];
        ...
        f_manager.removeBack();  //还未初始化，只是单纯的移除最老的图像帧，更新第一次观察到路标点的图像帧的索引  tzhang
}
